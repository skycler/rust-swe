...
int main() {
    std::cout << "Radial Dam Break Example\n";
    std::cout << "========================\n\n";
    ...
    // Setup radial dam break: circular high water region
    const auto& triangles = mesh->get_triangles();
    std::vector<swe::State> initial_state(triangles.size());
    
    const swe::Real center_x = width / 2.0;
    const swe::Real center_y = height / 2.0;
    const swe::Real radius = 3.0;
    
    for (size_t i = 0; i < triangles.size(); ++i) {
        swe::Real x = triangles[i].centroid.x;
        swe::Real y = triangles[i].centroid.y;
        
        swe::Real r = std::sqrt((x - center_x) * (x - center_x) + 
                               (y - center_y) * (y - center_y));
        
        if (r < radius) {
            initial_state[i] = swe::State(3.0, 0.0, 0.0);  // High water inside
        } else {
            initial_state[i] = swe::State(0.5, 0.0, 0.0);  // Low water outside
        }
    }
    ...
    // Setup output
    swe::VTKWriter writer("radial_dam");

    // Time stepping
    const swe::Real total_time = 3.0;
    const swe::Real output_interval = 0.05;
