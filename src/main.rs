...
use mesh::{TriangularMesh, TopographyType};
use solver::{ShallowWaterSolver, FrictionLaw};
use std::fs::File;
use std::io::Write;
...
fn main() {
    ...
    println!("  Grid points: {}x{} = {} triangles", 
             args.nx, args.ny, 2 * (args.nx - 1) * (args.ny - 1));
    ...
    let z_min = mesh.nodes.iter().map(|n| n.z).min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap_or(0.0);
    let z_max = mesh.nodes.iter().map(|n| n.z).max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap_or(0.0);
    ...
    match File::create(&filename) {
        Ok(mut file) => {
            ...
            writeln!(file, "CELLS {} {}", 
                     ...
                     solver.mesh.triangles.len() * 4).unwrap();
